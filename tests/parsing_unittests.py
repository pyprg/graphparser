...
from graphparser.parsing import (
    parse, parse2, _tokenize, _Token, _Tokencollection, _Attributetokens,
...
class Parse2(unittest.TestCase):

    def test_1_node(self):
        d = 'node'
        ex = [('node', 'node', (), {})]
        self.assertEqual([*parse2([d])], ex, f"expected {ex}")
    ...
    def test_2_nodes_no_edge(self):
        ...
        ex = [
            ('node', '0', (), {}),
            ('node', '1', (), {})]
        self.assertEqual([*parse2([d0])], ex, f"expected {ex}")
        self.assertEqual([*parse2([d1])], ex, f"expected {ex}")
        self.assertEqual([*parse2([d2])], ex, f"expected {ex}")
